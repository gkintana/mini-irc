...
void Server::handleNewConnections() {
	...
	std::cout << "ACCEPT BUFFER: " << buffer << std::endl;
	/*------------------------------------------------------------------*/
...
int Server::acceptConnections() {
	...
	return accept(m_server_fd, (struct sockaddr*)&m_address, &addrlen);
}
...
void Server::manageConnections(int i) {
	...
	this->changeNickname(buffer, src_fd);
...
// NICK Message Handler
void Server::changeNickname(char* buffer, int src_fd) {
	...
	if (client_msg == "NICK") {
		...
		res << ":" << this->m_nickname << " NICK " << new_nickname << "\r\n";
		this->m_nickname = new_nickname;
		send(src_fd, res.str().c_str(), strlen(res.str().c_str()), 0);
	}
}
